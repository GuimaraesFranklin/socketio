    ...
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('enviar_comando_serial', function (data) {
            ...
            if (data.options && data.options.portaSerial) {
                ...
                debug('enviando para porta serial');

                var isCaminhoDeRede = /^\/\//.test(data.options.portaSerial.match());
                if (isCaminhoDeRede) {
                    fs.writeFile(data.options.portaSerial, str, function (err) {
                        if (err) {
                            emitirErro(socket, err);
                            return;
                        }
                    ...
                    var port = new SerialPort(data.options.portaSerial, function (err) {
                        if (err) {
